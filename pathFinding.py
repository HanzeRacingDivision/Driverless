...
def makePath(mapToUse):
    ...
    if(len(mapToUse.target_list) == 0): #if there is no existing path to go on
        ...
        for LorR in range(2):
            if(firstCones[LorR] is None):
                ...
                for i in range(len(firstConeCandidates)):
                    ...
                    connectionAnglesAllowed = [((abs(GF.radDiff(cone.coneConData[0].angle, mapToUse.car.angle)) < pathFinder.pathFirstLineCarAngleDeltaMax) if (connectionCount > 0) else False), ((abs(GF.radDiff(cone.coneConData[1].angle, mapToUse.car.angle)) < pathFinder.pathFirstLineCarAngleDeltaMax) if (connectionCount > 1) else False)]
                    ...
                    coneOverlapsCar, distToCar = mapToUse.car.distanceToCar(cone.position)
                    ...
                    if(connectionCount < 1):
                        ...
                        coneCandidateStrength *= 1.5-min(distToCar/pathFinder.pathFirstLineCarDist, 1)  #high distance, low strength. non-Linear (quadratic?). worst>0.5 best<1.5
                        coneCandidateStrength *= 1.5-abs(GF.radDiff(cone.coneConData[(0 if connectionAnglesAllowed[0] else 1)].angle, mapToUse.car.angle))/pathFinder.pathFirstLineCarAngleDeltaMax  #high angle delta, low strength. Linear. worst>0.5 best<1.5
                        ...
                        coneCandidateStrength *= 1.5-min(min(abs(GF.radDiff(coneAvgConnAngle, mapToUse.car.angle)), abs(GF.radDiff(GF.radInv(coneAvgConnAngle), mapToUse.car.angle)))/pathFinder.pathConnectionMaxAngleDelta, 1)
                        ...
                        if(LorR == 1): #TO BE IMPROVED, but i dont know quite how yet
                            ...
                            if(leftFirstConeConnectionCount > 0):
                                ...
                                coneCandidateStrength *= 1.5-min(min(abs(GF.radDiff(leftAvgConnAngle, tempPathAngle)), abs(GF.radDiff(GF.radInv(leftAvgConnAngle), tempPathAngle)))/pathFinder.pathConnectionMaxAngleDelta, 1)
        ...
        mapToUse.target_list.append(newTarget)
    else: #if len(target_list) > 0
        lastPathLine = mapToUse.target_list[-1] # -1 gets the last item in list, you could also use (len(target_list)-1)
        ...
        for LorR in range(2):
            ...
            for i in range(lastConnectionCount):
                ...
                #     if(lastCones[LorR].connections[i].ID == mapToUse.target_list[-2].coneConData.cones[LorR].ID): #only check second to last path point
                #         connectedConesProspectable[i] = False;
                for target in mapToUse.target_list: #look through the entire list of targets (you could also just look at mapToUse.target_list[-2])
            ...
            if((lastConnectionCount == 1) and (not connectedConesProspectable[0])):
                ...
                if(connectedConesProspectable[0] and connectedConesProspectable[1]):
                    ...
                    for i in range(lastConnectionCount):
                        angleDeltas.append(abs(GF.radDiff(lastCones[LorR].coneConData[i].angle, mapToUse.car.angle)))
                        connectionSeqLengths.append(mapToUse.getConeChainLen(lastCones[LorR].connections[i], lastCones[LorR])[0])
                    for i in range(lastConnectionCount):
                    ...
                    if(lastCones[LorR].connections[0].ID == mapToUse.target_list[0].coneConData.cones[LorR].ID): #if the 0th connection is the first Target
                        #about to go full-circle, use connections[0] as prospectCone
                        prospectConnectionIndex = 0
                    elif(lastCones[LorR].connections[1].ID == mapToUse.target_list[0].coneConData.cones[LorR].ID): #if the 0th connection is the first Target
        ...
        # mapToUse.debugLines = [] #clear debugLines
        # for LorR in range(2):
        #     mapToUse.debugLines.append([1, mapToUse.realToPixelPos(lastCones[LorR].position), [4, lastConeAvgConnAngle[LorR]], 1+LorR])
        #     mapToUse.debugLines.append([1, mapToUse.realToPixelPos(prospectCones[LorR].position), [4, prospectConeAvgConnAngle[LorR]], 1+LorR])
        ...
        for i in range(3):
            ...
            strengths[i] *= 1.5-min(pathWidths[i]/pathFinder.pathConnectionThreshold, 1) #strength based on distance, the threshold just determines minimum score, distance can be larger than threshold without math errors
            strengths[i] *= 1.5-min(min(abs(GF.radDiff(pathAngles[i], allAvgConnAngles[(0 if (i==0) else 2)])), abs(GF.radDiff(pathAngles[i], GF.radInv(allAvgConnAngles[(0 if (i==0) else 2)]))))/pathFinder.pathConnectionMaxAngleDelta, 1) #strength based on angle delta from lastLeftConePerpAngle, the maxAngleDelta just determines minimum score, angle can be larger than threshold without math errors
            strengths[i] *= 1.5-min(min(abs(GF.radDiff(GF.radInv(pathAngles[i]), allAvgConnAngles[(1 if (i==1) else 3)])), abs(GF.radDiff(GF.radInv(pathAngles[i]), GF.radInv(allAvgConnAngles[(1 if (i==1) else 3)]))))/pathFinder.pathConnectionMaxAngleDelta, 1) #strength based on angle delta from prospectRightConePerpAngle, the maxAngleDelta just determines minimum score, angle can be larger than threshold without math errors
