...
//#define constPowerMode  //not the best idea (power hungry + runs hot for minor perf gain (maybe))             TBD!!
  ...
  const uint8_t AS5600_SDApin = 18; //note: 1.5kOhm pullup resistors are strongly advised
  const uint8_t AS5600_SCLpin = 19;
  #define motENpin 25         //you could do a second (higher freq) PWM on this one to limit the power, but IDK if it'll work well (since threeWireHbridge sucks)
  const uint8_t motApin = 26; //the ESP will let you do PWM on any output pin at any frequency and resolution
  const uint8_t motBpin = 27; //i think 8bit PWM resolution is fine for an application like this.
...
const float absMaxPower = 1.00; //(range = 0.0~1.0) you can limit the motor power to prevent belt slipping (and bone breaking if you're holding the wheel)
...
int32_t findRangeEnd(bool powerDirection) { //find one of the movement range limits (by running into it, hopefully gently)
  ...
  while(((millis() - endFindStartTime) < endFindTimeout) && keepFinding) {
    ...
    currentPos = updatePos(currentPos, twoWireReadTwoBytes(0x0C)); // ~80us on arduino, ~130us on ESP32
    ...
    applyMotPow(endFindMotorPower);
    delayMicroseconds((endFindTargetLoopTime-5)-(micros()-loopStart)); //calibrated delay to make preferred sample interval
  }
...
void centerCalib() { //move the servo to both ends of the steering range to make its position absolute
  #ifdef debugCenterCalib
...
//    pinMode(AS5600_SCLpin, INPUT_PULLUP);
    twoWireSetup(1000000, AS5600_SDApin, AS5600_SCLpin); //on the ESP32 (almost) any pins can be I2C pins
    ...
    pinMode(SDA, INPUT_PULLUP); //A4    NOT SURE IF THIS INITIALIZATION IS BEST (external pullups are stringly recommended anyways)
    pinMode(SCL, INPUT_PULLUP); //A5
    twoWireClockSetup(800000); //anything beyond 800kHz doesnt seem to work properly
  #endif
...
void loop() {
  ...
  int16_t newAngle = twoWireReadTwoBytes(0x0C); // ~80us on arduino, ~130us on ESP32
