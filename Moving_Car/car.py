...
class Car:
    ...
    def update(self, dt):
        ...
        self.true_position += self.velocity.rotate(-self.angle) * dt
        self.position = self.true_position + Vector2(np.random.normal(loc=0, scale=1e-10),
                                                     np.random.normal(loc=0, scale=1e-10))
