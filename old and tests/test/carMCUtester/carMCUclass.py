...
class carMCUserialProcess(MP.Process):
    ...
    def run(self):
        try:
            ...
            sendSerialDataToMaster = lambda newData : self.connToMaster.send(newData) #a simple callback function to send all (valid) received car feedback data to the master
            carConn = carMCUserial(self.comPort, self.autoFind, initMap.clock, sendSerialDataToMaster) #you could also initialize with "time.time)" as the clock func, you'd just need to let
            while(self.keepalive.get_value()):
                try:
                    while((not carConn.carMCUserial.is_open) and self.autoreconnect and self.keepalive.get_value()):
                    ...
                    while(carConn.carMCUserial.is_open and self.keepalive.get_value()):
                        ...
                        if(self.connToMaster.poll()):
                            ...
                            #if(type(dataToSend) is tuple): #extra safety checks
...
class carMCUserial:
    ...
    ## constants:
    sendMinInterval = 0.009 #minimum time between sending things (to avoid spamming the poor carMCU)
    ...
    def __init__(self, comPort=None, autoFind=True, clockFunc=time.time, customCallback=None): #if no clock function is supplied, time.time is used
    ...
    def sendSpeedAngle(self, speed, angle): #NOTE: 'angle' input for this function should be in radians
        ...
        if(abs(angle) > Map.Car.maxSteeringAngle): #extra safety/debugging
            ...
            if(self.carMCUserial.in_waiting > 0):
                ...
                for i in range(parsedPacketCount):
                    ...
                    if(self.customCallback):
                        #print("sending to master:", (self.feedbackTimestampFIFO[0], self.angleFIFO[0], self.distTotalFIFO[0]))
                        self.customCallback((self.feedbackTimestampFIFO[0], self.angleFIFO[0], self.distTotalFIFO[0]))
...
class realCar(Map.Car):
    """ a TEMPORARY class that uses carMCU sensor feedback to get car state (position, velocity, etc.)
        (overwrites Map.Car.update()) """
    def update(self, dTime, dDist): #this update() overwrites the update() in Map.Car, but this doesnt use the dt argument (because timestamps from the FIFO are used). inputDT is to removed in a future version
        ...
        if((abs(self.steering) > 0.001) and (abs(self.velocity) > 0.001)): #avoid divide by 0 (and avoid complicated math in a simple situation)
            ...
            self.position[0] = self.position[0] + (dDist * np.cos(self.angle))
...
if __name__ == '__main__':
    ...
    masterSharedMem.pickle(masterMap)
    ...
    carSerial = carMCUserialProcess(connForSerial, masterSharedMem.passToProcess, comPort="COM5", autoFind=False, autoreconnect=True)
    carSerial.start()
    print(masterMap.clock())
    while(masterMap.clock() < 20):
        connToCar.send((masterMap.car.desired_velocity, masterMap.car.desired_steering))
        if(connToCar.poll()):
            receivedData = connToCar.recv()
            #print("got from car:", receivedData)
        else:
            time.sleep(0.05)
    carSerial.stopLoop()
    carSerial.join()
