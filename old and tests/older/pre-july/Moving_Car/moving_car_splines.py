...
class PathPlanning:
    ...
    def run(self):
        ...
        def load_map(mouse_pos_list):
            ...
            map_path = os.path.join(current_dir, f"{name}.xlsx")
        ...
        while not self.exit:
            ...
            if pressed[pygame.K_l]:
                ...
                if mouse_pos in mouse_pos_list:
                    ...
                    make_cone = True
                    for i in range(len(mouse_pos_list)):
                        #if np.linalg.norm(tuple(x-y for x,y in zip(mouse_pos_list[i],mouse_pos))) < 50: #x and y are not x-coordinate and y-coordinate, but x1 and x2, and y1 and y2 on the next itteration. needlessly complicated?!?
                        if np.linalg.norm([mouse_pos_list[i][0]-mouse_pos[0], mouse_pos_list[i][1]-mouse_pos[1]]) < 50:
                            make_cone = False
                            break
