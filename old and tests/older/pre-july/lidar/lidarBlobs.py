...
def blobify(point, origin=None, clockFunc=time.time):
    """deletes-, appends to- or creates blobs given a lidar measurement (position, origin(optional))"""
    global blobList
    #checkDeleteBlobs(point, clockFunc)
    makeNewBlob = True
    if(len(blobList) > 0):
        makeNewBlob = not blobList[-1].append(point, origin, clockFunc) #try to add point to latest blob
    if(makeNewBlob): #if the list is empty, or appending failed
        blobList.append(blob(point, origin, clockFunc)) #add to total list
    checkBlobOverlap()
    checkBlobAge(clockFunc)
