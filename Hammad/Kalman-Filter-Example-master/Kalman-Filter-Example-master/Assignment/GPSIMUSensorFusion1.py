...
# Not changed (look at notation link)
...
# DataFileName and Values File Handles out

#  No idea why data or timestamp are here. These are empty lists
...
print("Reading files...")
# I assume we are making sensor fusion of each datapoint individually
...
# Is this a list of difference between next gps stamp - current * 0.000...01?? Bc if so which prediction are we making and for what
...
heading = MadgwickAHRS(sampleperiod=mean(dt)) #getting the heading out of the mean gps values?
...
for i in range(len(dt)):
    #we make prediction with IMU data
    ...
    #we update with prediction data, gps lon+lat, measurement cov, measurement matrix??
