...
def save_map(left_cones, right_cones):
    ...
    map_file = pd.DataFrame({'Cone_Type': cone_type, 'Cone_X': cone_x, 'Cone_Y': cone_y})
...
def update_target_lists(targets, non_passed_targets):
    # list of visible targets and non-passed targets
    dists = []
    non_passed_dists = []
    visible_targets = []
    visible_dists = []
...
def update_cone_lists(left_cones, right_cones, visible_left_cones, visible_right_cones):
    ...
    return visible_left_cones, visible_right_cones, len_visible_left_cones_new, len_visible_right_cones_new, \
        new_visible_left_cone_flag, new_visible_right_cone_flag
...
def closest_target(visible_targets, visible_dists):
    ...
    if len(visible_targets) > 0:
        if len(visible_dists) == 0:
            ...
            closest = visible_targets[np.array(visible_dists).argmin()]
    return closest, visible_targets
