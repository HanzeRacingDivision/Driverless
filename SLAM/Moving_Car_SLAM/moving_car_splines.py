...
class Cone:
    ...
    def update(self, car, time_running, ppu, car_angle):
        ...
        if np.linalg.norm(
                self.position - car.true_position) < car.fov / ppu and self.visible == False and car.auto == True:
...
class PathPlanning:
    ...
    def run(self):
        ...
        while not self.exit:
            ...
            if pressed[pygame.K_l]:
                ...
                if mouse_pos in mouse_pos_list:
                    ...
                    if make_cone == True:
                        ...
                        left_cones.append(left_cone)
                        landmarks_left = np.vstack(
                            (landmarks_left, np.array([left_cone.position.x, left_cone.position.y])))
                        mouse_pos_list.append(mouse_pos)
            ...
            if pressed[pygame.K_r]:
                ...
                if mouse_pos in mouse_pos_list:
                    ...
                    if make_cone == True:
                        ...
                        right_cones.append(right_cone)
                        landmarks_right = np.vstack(
                            (landmarks_right, np.array([right_cone.position.x, right_cone.position.y])))
                        mouse_pos_list.append(mouse_pos)
            ...
            if first_visible_left_cone != 0 and first_visible_right_cone != 0 and np.linalg.norm(
                    (start_midpoint_x, start_midpoint_y) - car.position) < 20 / ppu and track_number_changed == False:
                track_number += 1
                track_number_changed = True

            elif first_visible_left_cone != 0 and first_visible_right_cone != 0 and np.linalg.norm(
                    (start_midpoint_x, start_midpoint_y) - car.position) > 20 / ppu:
                track_number_changed = False
            ...
            if fullscreen == False:
                ...
                text_surf = text_font.render(f'True [X, Y, alpha]:'
                                             f' {[round(car.position.x, 2), round(car.position.y, 2)]}', 1,
                                             (255, 255, 255))
                text_pos = [10, 130]
                self.screen.blit(text_surf, text_pos)
                # SLAM pose print
                ...
                if slam:
                    text_surf = text_font.render(f'Est. [X, Y, alpha]:'
                                                 f' {[round(float(x_state[0]), 2), round(float(x_state[1]), 2)]}', 1,
                                                 (255, 255, 255))
                text_pos = [10, 150]
                self.screen.blit(text_surf, text_pos)
