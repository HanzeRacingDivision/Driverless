...
import pandas as pd
from ekf_slam import STATE_SIZE, observation, ekf_slam


class Car:
    def __init__(self, x, y, angle=0, length=2, max_steering=20, max_acceleration=2.0):
...
class SLAM:
    ...
    def run(self):
        ...
        # needed here to make sure we give an angle between -PI, PI as a starting angle
        ...
        # here both xEST (estimate of SLAM) and xTrue are initialized
        ...
        # PEst is the covariance matrix of all values (Pose and Cones)
        PEst = np.eye(STATE_SIZE)
        x_state = xEst
        # history might be useful for plotting later on
        hxEst = xEst
        hxTrue = xTrue
        hxDR = xTrue
        ...
        # even tho the visible cones thing seems to work still kinda useful if we wanna use all cones 
        # there is a check in ekf_slam observation so that only cones in a certain radius are used
        ...
        while not self.exit:
            ...
            # some check for linear and angular velocity I added 
            ...
            # if we use all landmarks initally, there is a check in observation for given range as well
            ...
            if car.slam:
                ...
                if "landmarks" in dir():
                    ...
                    # print("Input to SLAM", xEst, PEst, U, landmark_measurements, dt)
            ...
            # car.slam cone drawing should work properly this way but might still be some error
            # the way cone locations are stored is: Its all in one vector xEst the first three entries
            # are the POSE (x, y, yaw) then every tuple afterwards is a estimated cone location (so 4th + 5th)
            # entry is x, y of first cone etc
