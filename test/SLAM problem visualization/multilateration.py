...
def findMinimumErrorsSquared(initialGuess: np.ndarray, origins: np.ndarray, measuredRadii: np.ndarray, measurementGains: np.ndarray, axis: int):
    ...
    posAtMinima = changeOneAxis(secant(secantFunc, 0.0001, 5, secantFunc(initialGuess[axis]), initialGuess[axis], firstAxisStep), initialGuess, axis)
...
# def multilaterate(initialGuess: np.ndarray, origins: np.ndarray, measuredRadii: np.ndarray, measurementGains: np.ndarray):
#     """find the best approximation of the intersection between several circles (defined by origins and radii).
#         the math used comes from this paper: http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.966.9430&rep=rep1&type=pdf
#         can be used to find a position estimate for a sensor system that gives directionless distance measurements
#         requires initialGuess, but doesnt have to be exact"""
#     maxItt = 10
#     stoppingTolerance = 0.00001
...
#             vectLen = math.hypot(*JacobUnitVects[j])
#             if(vectLen > 0.00001):
#                 JacobUnitVects[j] /= vectLen #make into unit vector (normalize length to 1.0)
#             else:
...
#     result = initialGuess.copy()
#     print("\n start:", result)
#     b = np.array([(measuredRadii[i]**2)-(origins[i][0]**2 + origins[i][1]**2) for i in range(len(origins))])
#     for itt in range(maxItt):
#         A = np.array([result-(2*origin) for origin in origins])
#         #print("A:", A)
