...
def getCones(previewQueue, detectionNNQueue, depthQueue, networkQueue, xoutBoundingBoxDepthMappingQueue, labelMap, start, out):
    ...
    frame_aug = cv2.putText(frame, "NN fps: {:.2f}".format(fps), (2, frame.shape[0] - 4), cv2.FONT_HERSHEY_TRIPLEX, 0.4, color)
...
class DetectionMoule:
    ...
    nnBlobPath = "/home/catalinzaharia/Development/HARD/Driverless/processing/computer_vision/YOLOv5/shaves/416_half_shave_summer_FSE2022.blob"
     ...
     return pipeline
        ...
        while True:
            getCones(previewQueue, detectionNNQueue, depthQueue, networkQueue, xoutBoundingBoxDepthMappingQueue, labelMap, start, out)
            if cv2.waitKey(1) == ord('q'):
                break
