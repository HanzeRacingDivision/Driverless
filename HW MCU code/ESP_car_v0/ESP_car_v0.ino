...
const float motorKpAdjAvgDepth = 4; //number of points to consider when calculating average speed (MUST BE <= speedLogFIFOsize)
...
static const float servoMicrosPerDegree = 20.0; //(output) microseconds per degree (TO BE CALIBRATED)
#define servoPWMChan 0
static const float servoADCvalPerDegree = 20.0; //(input) ADCval per degree (TO BE CALIBRATED)
static const int16_t servoPotMid = 2048; //(input) center value (straight steering) for ADC data from servo potentiometer
...
const float serialInputAngleMin = -45.1; //minimum allowed angle input
const float serialInputAngleMax = 45.1; //maximum allowed angle input
...
const uint32_t serialFeedbackInterval = 300; //time between serial feedback messages (in millis)
const float serialFeedbackAvgSpeedDepth = 4; //number of points to consider when calculating average speed (MUST BE <= speedLogFIFOsize)
...
void loop() {
  ...
  if(theSerial.available()) {
    ...
    if(longEnoughInput) { //if the message length is >= minSerialInputLength
      ...
      if((serialInputAngle > serialInputAngleMin) && (serialInputAngle < serialInputAngleMax)) {
        servoTarget = servoMid + ((int16_t)(serialInputAngle * servoMicrosPerDegree)); //convert from degrees to servo PWM pulselength
  ...
  if(millis() >= serialFeedbackTimer) {
    ...
    float steeringAngle = ((float)(avgServo() - servoPotMid))/servoADCvalPerDegree; //convert from ADCval to degrees
    float serialFeedbackDrvnDist = (encoCount - serialFeedbackEncoCountLast) * encoCountPerDM; //determine distance driven since last feedback
    ...
    String dataString = String(avgSpeed(serialFeedbackAvgSpeedDepth) * 10.0, 2) + serialFeedbackSeperator + String(serialFeedbackDrvnDist, 2) + serialFeedbackSeperator + String(steeringAngle, 1);
